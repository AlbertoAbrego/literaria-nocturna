...
- **Backend**: Coverage report generated by `jest --coverage` (v8 provider). Output in `backend/coverage/` (git-ignored). No threshold configured yet.
- **Frontend**: Coverage available via `npm run test:coverage` (v8 provider). Not run in CI by default.
